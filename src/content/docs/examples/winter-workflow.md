...
Cold, harness, context, and documentation review also accept a remote PR or MR locator supported by an available, authenticated forge CLI. The skills bind per-axis feedback defaults — code findings post inline; the other axes return reports. Each axis document declares its remote-feedback behavior in its Output section; the [axes registry](https://github.com/paul-gross/winter-workflow/blob/master/methodology/review/axes/index.md) routes to every axis document. A caller using the review process's normalized semantic inputs directly may explicitly override `scope.feedback` to `report` or `inline`; the skills do not expose that override until their adapter syntax adds it. Retrieval or posting failures are reported and never silently replaced with a local-branch review.
